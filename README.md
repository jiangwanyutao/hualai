...
- 🧪 **写完就验证** — `/hualai:ceshi` 按这次改动、一句功能描述或一份进度文档生成前后端验证提示词：接口真实调用、页面真实操作并截图
- 🤖 **自动增强（可选）** — 发 `hualai on` 后，每条消息先经 hualai 改写再执行，不用再手敲命令；默认关闭

### 最新动态

* **[2026.09]** 🐛 自动增强的子进程不再保存会话，会话历史里不会每条消息多出一个 `hualai:hualai` 碎片会话（0.3.1）
* **[2026.09]** 🧪 新增 `/hualai:ceshi`：写完功能后，按这次改动生成前后端验证提示词
...
- 改写由一个独立的 `claude -p` 子进程（Sonnet）完成，每条消息多等 30–60 秒、多一次调用费用；失败时按原话执行并提示原因。子进程不保存会话（`--no-session-persistence`），不会出现在会话历史里；0.3.0 及更早版本会留下首条消息为 `hualai:hualai` 的碎片会话，升级后可手动删除。
...
### 写完功能后验证（/hualai:ceshi）

功能写完后敲一下，把要测的东西写在同一行：这次的改动、一个提交范围、一句功能描述，或一份进度文档都行。hualai 只读找出涉及的接口和页面，输出一段验证提示词，让 Claude 照着真实验证前后端：

```text
/hualai:ceshi                       # 未提交的改动；工作区干净时取最近一次提交
/hualai:ceshi HEAD~3                # 最近 3 次提交
/hualai:ceshi a1b2c3 只测前端        # 指定提交，后面可加补充说明
/hualai:ceshi 训练任务列表的状态筛选   # 一句功能描述：不看 git，直接在代码里找这个功能
/hualai:ceshi docs/进度.md          # 进度文档：只挑标了完成的项（[x] / ✅ / 已完成），逐项出验证步骤，最多 5 项
```

生成的提示词包含：

- **接口**：每个接口的方法、路径、代码位置，一个正常输入和至少一个异常输入；要求真实调用，贴状态码和返回内容
- **页面**：每个页面的路由、代码位置，触发这次改动的操作步骤和一个边界情况；要求用浏览器真实操作、截图，检查控制台报错和请求是否成功
- **怎么启动**：项目里真实存在的启动与测试命令
- **结果表**：每项「通过 / 失败 / 没测」加证据，跳过不算通过，失败先报告不顺手改

`/hualai:ceshi` 自己只读、只出提示词，不调接口、不开浏览器。只改了文档时，它会直接告诉你没有需要运行验证的内容。
